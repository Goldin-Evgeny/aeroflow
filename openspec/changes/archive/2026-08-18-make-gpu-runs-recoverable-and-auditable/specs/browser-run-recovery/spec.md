...
Unexpected browser closure, harness timeout, detected stall, device loss, and operator abort
SHALL preserve the most recent complete checkpoint. Checkpoint deletion SHALL occur only for an
explicit fresh-start or cleanup operation, not as an incidental consequence of failure.
...
context interruption. Validation of this capability SHALL NOT require reproducing the
intermittent long-run GPU stall or completing a multi-hour acceptance case.
...
- **THEN** the test SHALL demonstrate compatible-state restoration and forward progress from
  the committed step within the ordinary end-to-end test budget
